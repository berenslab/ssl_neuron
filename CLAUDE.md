...
**Eyewire2 retina adaptation** (`ssl_neuron/eyewire2/`): `dataset.py`'s `RetinaGraphDataset` subclasses `GraphDataset` and overrides `_augment_node_position`, swapping in `augment.py` (pure numpy, importable without `torch`), plus `_augment` to optionally clip each view at a simulated square volume edge (`data.crop_xy`, `random_crop_xy`) before the inherited branch deletion and subsampling. Two things differ from the ABA assumptions and will silently ruin a run if ignored:
